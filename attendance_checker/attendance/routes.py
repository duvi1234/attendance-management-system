...
def update_attendance():
    ...
    if request.method == 'POST':
        ...
        # Find all days in the form by looking for keys like date_{student_id}_{day}
        for key in request.form:
            if key.startswith('date_'):
                _, student_id, day = key.split('_', 2)
                student_id = int(student_id)
                date_str = request.form[key]
                # Check if this day is marked as a holiday
                holiday_key = f"holiday_{student_id}_{day}"
                is_holiday = holiday_key in request.form
                total_key = f"total_{student_id}_{day}"
                attended_key = f"attended_{student_id}_{day}"
                # Only process if not a holiday and total is provided
                if not is_holiday and request.form.get(total_key):
                    ...
                    if existing_attendance:
                        existing_attendance.total_classes = total
                        existing_attendance.attended_classes = attended
                    else:
                    ...
                    # Optionally, you could store a record for holidays with total/attended as 0
                    pass
        db.session.commit()
    ...
    return render_template('update_attendance.html', students=students_data, subjects=subjects, active_semester=active_semester)
